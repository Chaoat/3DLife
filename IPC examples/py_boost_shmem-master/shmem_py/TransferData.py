...
class TransferData(ctypes.Structure):
    def __init__(self):
        super().__init__()
        self.fields = [
        ('cells', ctypes.c_uint * self.cellsPerDimension[-1])
        ('drawMode', ctypes.c_bool)
        ('dimensions', ctypes.c_uint * len(self.dimensions))
    ]

class TransferData(ctypes.Structure):
_fields_ = [
    ('cells', ctypes.c_uint * cellsPerDimension[-1])
    ('drawMode', ctypes.c_bool)
    ('cells', ctypes.c_uint * len(self.dimensions))
]
